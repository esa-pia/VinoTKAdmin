ActiveAdmin.register Millesime do
  ...
  form do |f|                         
    f.inputs I18n.t('millesimes.section_title') do       
      ...
      f.input :bouteilles, :label => I18n.t('millesimes.bouteilles'), :input_html => { :class => 'chzn-select', :width => 'auto', "data-placeholder" => I18n.t('millesimes.choose.bouteilles') }, :collection => (Bouteille.order('type_id ASC , appellation ASC, domaine_id ASC, cuvee_id ASC').all).map{|o| [ "#{o.type.libelle} - #{o.appellation} - #{o.domaine.libelle} - #{o.cuvee.libelle} - #{o.volume.valeur}- #{o.millesime.valeur}", o.id]}
    end                               
    f.actions                         
  end
